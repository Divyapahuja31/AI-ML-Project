...
\begin{lstlisting}[caption={Random Forest hyperparameters.}]
...
\text{Precision} &= \frac{TP}{TP + FP}
    \quad \text{(of all flagged, how many truly default?)} \\[4pt]
\text{Recall}    &= \frac{TP}{TP + FN}
    \quad \text{(of all true defaults, how many did we catch?)} \\[4pt]
F_1              &= \frac{2 \cdot \text{Precision} \cdot \text{Recall}}
                       {\text{Precision} + \text{Recall}} \\[4pt]
...
\midrule
Accuracy   & $\approx 0.78$ & Misleading due to class imbalance \\
Precision  & $\approx 0.31$ & Of flagged applicants, 31\% truly default \\
Recall     & $\approx 0.72$ & Captures 72\% of actual defaults \\
$F_1$      & $\approx 0.43$ & Harmonic mean of Precision and Recall \\
ROC-AUC    & $\approx 0.86$ & Strong discriminative ability \\
CV ROC-AUC & $0.86 \pm 0.01$ & 5-fold cross-validation (stable) \\
\bottomrule
\end{tabular}
\end{table}
...
\bibitem{kaggle2024}
Shamim, A. (2024).
\textit{Credit Risk Benchmark Dataset}.
Kaggle.
...
In \textit{Proceedings of the AAAI Workshop on Imbalanced Data Sets}.

\bibitem{langgraph2024}
LangChain Inc. (2024).
\textit{LangGraph: A framework for building stateful, multi-actor applications with LLMs}.
